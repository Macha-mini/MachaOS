...
**2026-08-06 追記 3 — flaky ハングは再現せず(解決した可能性が高い)**:

Phase 8b (SIGCHLD 含むシグナル配送) 実装後 + `sys_wait4` の完全回収化
(追記 4 参照) の後、クリーンビルドで `make test` を **6 回連続実行し
全て [SELFTEST OK]**。`busybox sh exited normally` が毎回確認でき、
「クリーンビルド 3/3 FAIL」だった頃の症状は再現しなくなった。

嫌疑は以下の2つで、どちらもこの間に塞がっている:
1. **特定 pid の waitpid が親チェックなしで古いゾンビを誤マッチ**:
   PHASE7D の item 6 修正は wait4(-1) の親チェックのみで、`waitpid(pid)`
   の特定 pid 経路には親チェックが無かった。ash がジョブの pid を
   wait した際、過去テストの未回収ゾンビが誤って exit 済みとして返り
   ash のジョブブックキーピングが壊れる — タイミング依存だったのは
   ゾンビの pid が ash の予想と一致した時だけだから。A.1 で特定 pid
   にも親チェックを追加し、かつ wait4 が子を完全回収するようになった
   ためこの経路は消滅。
2. **シグナル配送 (Phase 8b)**: ash は SIGCHLD ハンドラを登録しており、
   未配送のままの時代より待機経路が完成に近づいた。

**結論**: 次のセッションで再発するか数回 `make test` で確認する程度で
よく、gdbstub 調査は当面不要。再発した場合はまず A.1 の wait4 変更
(特定 pid 親チェック + 完全回収) を疑うこと。

**2026-08-06 追記 4 — wait4 が子を完全回収するよう修正 (ゾンビリーク解消)**:
`sys_wait4` (`src/linux_abi.rs`) は exit 状態をクリアするだけで task
スロットと全フレーム (ページテーブル・セグメント・1MiB スタック領域)
を解放しておらず、busybox sh のパイプライン内コマンドなど内部 fork の
子が永久にリークしていた。wait4 がマッチした子を `process::reap` で
実際に削除するように変更。
- ECHILD 判定を「子が1つも無い場合」にも拡張 (子を削除する方式に
  変わったため、`waitpid(-1)` のポーリング終端が ECHILD で止まる)
- 特定 pid の wait にも親子チェックを追加 (Linux では非子は ECHILD。
  誤マッチで他プロセスのアドレス空間を解放しないため必須)
- `task::remove_process` が mid-table 削除時のインデックス整合を維持:
  `parent`/`thread_of` 参照の補正、削除されたプロセスの死んだ
  CLONE_VM スレッドの掃除、孤児プロセスの parent=None 化
