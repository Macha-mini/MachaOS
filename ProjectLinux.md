...
## Phase 6 — 動的リンクの完成(最優先・全ての前提)【完了 — 実glibc動的リンク動作】

実Debian 12 (glibc 2.36) の動的リンクバイナリ **hello / coreutils true / cat の3つがすべて最後まで実行完了**し、hello は本物の `printf` 経由で "Hello, world!" を出力する。Phase 4 の「既知の未解決ギャップ」は解消。`make test` 回帰スイートも全項目パス。

**最終バグの根本原因と修正**:

1. **`translate()` のVMAマッチングが「最初にマッチ」だった【修正済み・根本原因】**: ld.so は共有ライブラリを「全体スパンを先に `mmap` → 各 `PT_LOAD` を `MAP_FIXED` で上書き」という順序でマップする。ページテーブルは**最後のマップ**が勝つ(`map_range_in` がPTEを上書き)が、`process::translate`/`translate_local` は**最初にマッチするVMA**を返していたため、カーネル側のリロケーション書き込み(後述のworkaround群)はすべて実行時にCPUが読まない物理フレーム(旧スパンマップのコピー)に飛んでいた。`rev().find()` で「最新VMA優先」に修正(コミット`d6aa092`)。
2. **ADJUST workaround撤去**: ld.so の `D_PTR` は自前で `.dynamic` のアドレスタグをバイアスする(link_map+0x336 の bit 0x20 が立っているため `l_addr` 加算)。カーネルが mmap 時にバイアスすると**二重バイアス**(0x4338 → 0x80004338、`_dl_setup_hash` 内でフォールト)になるため撤去。
3. **RELR workaround撤去**: カーネルのデコーダはビット意味が**逆**だった(glibc 2.36 実装では **bit0=0 がアドレスエントリ、bit0=1 が前の`where`から続くビットマップ**)。誤デコード + ld.so 自身の RELR 適用との二重適用で TLS イメージ(0x1cf8d0)が二重バイアスされ `__ctype_init` が 0x801d4560 をデリファレンスしてフォールト。撤去により ld.so の正しい RELR 処理に一本化。
4. **SYMBOLIC/PLT workaround撤去**: translate 修正後、ld.so の `_dl_relocate_object` が libc に対して完走するようになった(RELA = GLOB_DAT/R_X86_64_64/TPOFF64 + RELR + レイジーJUMP_SLOT)。カーネルの eager バインドは ld.so の RELATIVE パスと競合して GOT が二重バイアス(0x38014060 → 0x78014060)されるため撤去。
5. **Phase 6 temp デバッグログ撤去**: シリアルログが 275KB → 7.5KB に縮小。

**検証**:
- 主目標達成: GNU Hello が最後まで実行完了し "Hello, world!" を出力(`make test` のシリアルログで確認済み)。
- 実 coreutils `true` / `cat` も正常終了(hello と同じ実行経路)。
- 回帰: 静的musl/busybox、ネイティブABI、Wayland、ネットワークの全 selftest 項目パス。
