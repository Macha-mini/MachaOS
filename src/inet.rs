...
pub fn connect_done(id: usize) -> Result<bool, i32> {
    ...
    match t.state {
        ...
        TcpState::SynSent | TcpState::FinSent => {
            if ep.rst {
                Err(ECONNREFUSED)
            } else {
                Ok(false)
...
pub fn readiness(id: usize) -> (bool, bool) {
    ...
    let read = !ep.inbox.is_empty() || ep.eof || ep.rst || !ep.udp_inbox.is_empty();
...
fn dispatch_tcp(src_ip: [u8; 4], payload: &[u8]) {
    ...
    let Some(ep) = state
        ...
        return; // no such connection — drop (no RST, documented limitation)
    };
